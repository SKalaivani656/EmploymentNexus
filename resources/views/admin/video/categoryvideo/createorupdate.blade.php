...
@section('adminnavbar')
    <x-admin.layouts.adminnavbar modulename="VIDEO CATEGORY" />
@endsection
...
<x-admin.layouts.adminbreadcrumb>
    ...
    <li class="breadcrumb-item active" aria-current="page">
        <a class="text-decoration-none" href="{{ route('categoryvideo.index') }}">Video Category</a>
    </li>
    <li class="breadcrumb-item active" aria-current="page">{{ (!$categoryvideo->id) ? 'Create':  'Update'}} </li>
</x-admin.layouts.adminbreadcrumb>
...
<x-admin.layouts.admincreateorupdate>
    ...
    <x-slot name="action">
        <a class="btn btn-sm btn-secondary shadow float-end" href="{{ route('categoryvideo.index') }}" role="button">Back</a>
    </x-slot>

    <x-slot name="form">
    {!! Form::model($categoryvideo, ['route' => ['categoryvideo.store', $categoryvideo->id],  'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true','enctype'=>'multipart/form-data']) !!}
    {{ Form::hidden('id', $categoryvideo->id, array('id' => 'invisible_id')) }}
    {{ Form::hidden('uniqid', $categoryvideo->uniqid, array('id' => 'invisible_id')) }}

    @include('admin.video.categoryvideo.form')
...
<script>
  ...
  $("#categoryvideo").css({"background-color": "#00c5d0"});
