...
<x-admin.layouts.adminbreadcrumb>
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ (!$categoryblog->id) ? 'Create':  'Update'}} </li>
</x-admin.layouts.adminbreadcrumb>
...
<x-admin.layouts.admincreateorupdate>
    ...
    <x-slot name="action">
        <a class="btn btn-sm btn-secondary shadow float-end" href="{{ route('categoryblog.index') }}" role="button">Back</a>
    </x-slot>

    <x-slot name="form">
    {!! Form::model($categoryblog, ['route' => ['categoryblog.store', $categoryblog->id],  'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true','enctype'=>'multipart/form-data']) !!}
    {{ Form::hidden('id', $categoryblog->id, array('id' => 'invisible_id')) }}
    {{ Form::hidden('uniqid', $categoryblog->uniqid, array('id' => 'invisible_id')) }}

    @include('admin.blog.categoryblog.form')
