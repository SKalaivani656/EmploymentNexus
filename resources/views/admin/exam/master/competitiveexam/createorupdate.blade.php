...
<x-admin.layouts.adminbreadcrumb>
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ (!$competitiveexam->id) ? 'Create':  'Update'}} </li>
</x-admin.layouts.adminbreadcrumb>
...
<x-admin.layouts.admincreateorupdate>
    ...
    <x-slot name="action">
        <a class="btn btn-sm btn-secondary shadow float-end" href="{{ route('competitiveexam.index') }}" role="button">Back</a>
    </x-slot>

    <x-slot name="form">
    {!! Form::model($competitiveexam, ['route' => ['competitiveexam.store', $competitiveexam->id],  'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true','enctype'=>'multipart/form-data']) !!}
    {{ Form::hidden('id', $competitiveexam->id, array('id' => 'invisible_id')) }}
    {{ Form::hidden('uniqid', $competitiveexam->uniqid, array('id' => 'invisible_id')) }}

    @include('admin.exam.master.competitiveexam.form')

    <div class="text-center mt-3">
    @if ($competitiveexam->id)
