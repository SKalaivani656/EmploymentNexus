...
<x-admin.layouts.adminbreadcrumb>
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ (!$rolejob->id) ? 'Create':  'Update'}} </li>
</x-admin.layouts.adminbreadcrumb>
...
<x-admin.layouts.admincreateorupdate>
    ...
    <x-slot name="action">
        <a class="btn btn-sm btn-secondary shadow float-end" href="{{ route('rolejob.index') }}" role="button">Back</a>
    </x-slot>

    <x-slot name="form">
    {!! Form::model($rolejob, ['route' => ['rolejob.store', $rolejob->id],  'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true','enctype'=>'multipart/form-data']) !!}
    {{ Form::hidden('id', $rolejob->id, array('id' => 'invisible_id')) }}
    {{ Form::hidden('uniqid', $rolejob->uniqid, array('id' => 'invisible_id')) }}

    @include('admin.job.jobmaster.rolejob.form')
...
<script type="text/javascript">
 ...
 $( document ).ready(function() {
   ...
   function checkCountTitle() {
     textLengthTitle = $('#role').val().length;
