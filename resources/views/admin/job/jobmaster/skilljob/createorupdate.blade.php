...
@section('adminnavbar')
    <x-admin.layouts.adminnavbar modulename="SKILL" />
@endsection
...
<x-admin.layouts.adminbreadcrumb>
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ (!$skilljob->id) ? 'Create':  'Update'}} </li>
</x-admin.layouts.adminbreadcrumb>
...
<x-admin.layouts.admincreateorupdate>
    ...
    <x-slot name="action">
        <a class="btn btn-sm btn-secondary shadow float-end" href="{{ route('skilljob.index') }}" role="button">Back</a>
    </x-slot>

    <x-slot name="form">
    {!! Form::model($skilljob, ['route' => ['skilljob.store', $skilljob->id],  'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true','enctype'=>'multipart/form-data']) !!}
    {{ Form::hidden('id', $skilljob->id, array('id' => 'invisible_id')) }}
    {{ Form::hidden('uniqid', $skilljob->uniqid, array('id' => 'invisible_id')) }}

    @include('admin.job.jobmaster.skilljob.form')
