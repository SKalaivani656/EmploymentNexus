...
@section('adminnavbar')
    <x-admin.layouts.adminnavbar modulename="TYPE" />
@endsection
...
<x-admin.layouts.adminbreadcrumb>
    <li class="breadcrumb-item active" aria-current="page">
        <a class="text-decoration-none" href="{{ route('typejob.index') }}">Master</a>
    </li>
    <li class="breadcrumb-item active" aria-current="page">
        <a class="text-decoration-none" href="{{ route('typejob.index') }}">Type</a>
    </li>
    <li class="breadcrumb-item active" aria-current="page">{{ (!$typejob->id) ? 'Create':  'Update'}} </li>
</x-admin.layouts.adminbreadcrumb>
...
<x-admin.layouts.admincreateorupdate>
    ...
    <x-slot name="action">
        <a class="btn btn-sm btn-secondary shadow float-end" href="{{ route('typejob.index') }}" role="button">Back</a>
    </x-slot>

    <x-slot name="form">
    {!! Form::model($typejob, ['route' => ['typejob.store', $typejob->id],  'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true','enctype'=>'multipart/form-data']) !!}
    {{ Form::hidden('id', $typejob->id, array('id' => 'invisible_id')) }}
    {{ Form::hidden('uniqid', $typejob->uniqid, array('id' => 'invisible_id')) }}

    @include('admin.job.jobmaster.typejob.form')
...
<script type="text/javascript">
 ...
 $( document ).ready(function() {
   ...
   function checkCountTitle() {
     textLengthTitle = $('#type').val().length;
...
<script>
  ...
  $("#typejob").css({"background-color": "#00c5d0"});
