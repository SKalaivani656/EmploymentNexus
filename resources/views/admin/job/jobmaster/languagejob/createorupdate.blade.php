...
<x-admin.layouts.adminbreadcrumb>
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ (!$languagejob->id) ? 'Create':  'Update'}} </li>
</x-admin.layouts.adminbreadcrumb>
...
<x-admin.layouts.admincreateorupdate>
    ...
    <x-slot name="action">
        <a class="btn btn-sm btn-secondary shadow float-end" href="{{ route('languagejob.index') }}" role="button">Back</a>
    </x-slot>

    <x-slot name="form">
    {!! Form::model($languagejob, ['route' => ['languagejob.store', $languagejob->id],  'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true','enctype'=>'multipart/form-data']) !!}
    {{ Form::hidden('id', $languagejob->id, array('id' => 'invisible_id')) }}
    {{ Form::hidden('uniqid', $languagejob->uniqid, array('id' => 'invisible_id')) }}

    @include('admin.job.jobmaster.languagejob.form')
...
<script type="text/javascript">
 ...
 $( document ).ready(function() {
   ...
   function checkCountTitle() {
     textLengthTitle = $('#company').val().length;
