...
<x-admin.layouts.adminbreadcrumb>
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ (!$designationjob->id) ? 'Create':  'Update'}} </li>
</x-admin.layouts.adminbreadcrumb>
...
<x-admin.layouts.admincreateorupdate>
    ...
    <x-slot name="action">
        <a class="btn btn-sm btn-secondary shadow float-end" href="{{ route('designationjob.index') }}" role="button">Back</a>
    </x-slot>

    <x-slot name="form">
    {!! Form::model($designationjob, ['route' => ['designationjob.store', $designationjob->id],  'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true','enctype'=>'multipart/form-data']) !!}
    {{ Form::hidden('id', $designationjob->id, array('id' => 'invisible_id')) }}
    {{ Form::hidden('uniqid', $designationjob->uniqid, array('id' => 'invisible_id')) }}

    @include('admin.job.jobmaster.designationjob.form')

    <div class="text-center mt-3">
    @if ($designationjob->id)
...
<script type="text/javascript">
 ...
 $( document ).ready(function() {
   ...
   function checkCountTitle() {
     textLengthTitle = $('#designation').val().length;
