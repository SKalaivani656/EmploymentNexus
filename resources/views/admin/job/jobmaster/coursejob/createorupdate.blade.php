...
@section('main-content')
    ...
    <x-admin.layouts.adminbreadcrumb>
        ...
        <li class="breadcrumb-item active" aria-current="page">{{ !$coursejob->id ? 'Create' : 'Update' }} </li>
    </x-admin.layouts.adminbreadcrumb>
    ...
    <x-admin.layouts.admincreateorupdate>
        ...
        <x-slot name="form">
            {!! Form::model($coursejob, ['route' => ['coursejob.store', $coursejob->id], 'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true', 'enctype' => 'multipart/form-data']) !!}
            {{ Form::hidden('id', $coursejob->id, ['id' => 'invisible_id']) }}
            {{ Form::hidden('uniqid', $coursejob->uniqid, ['id' => 'invisible_id']) }}

            @include('admin.job.jobmaster.coursejob.form')
...
@section('footerSection')
    ...
    <script>
        $(document).ready(function() {
            ...
            function ajaxbranchjob() {
                $.ajax({
                    url: "{{ route('ajaxbranchjob') }}",
                    mehtod: "get",
                    dataType: 'json',
                    success: function(data) {

                        $('#branchoption').html(data.branchoption);
                        $('#branchoption').val({!! $coursejob->branchSelect !!});
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            function checkCountTitle() {
                textLengthTitle = $('#course').val().length;
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            /* initalize for when no data is in localStorage */
            var countTitle = maxCharstitle;
            $('#onlynameLeft').text('ONLY NAME' + countTitle + ' characters left (Min:20,Max:80)');
            ...
            function checkCountTitle() {
                textLengthTitle = $('#onlyname').val().length;
                if (textLengthTitle >= maxCharstitle) {
                    $('#onlynameLeft').text(outOfChars);
                } else {
                    countTitle = maxCharstitle - textLengthTitle;
                    $('#onlynameLeft').text('ONLY NAME' + countTitle + ' characters left (Min:20,Max:80)');
                }
            ...
            /* on keyUp: update #onlynameLeft as well as count & commentTitle in localStorage */
