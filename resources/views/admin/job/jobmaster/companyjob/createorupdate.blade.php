...
@section('main-content')
    ...
    <x-admin.layouts.adminbreadcrumb>
        ...
        <li class="breadcrumb-item active" aria-current="page">{{ !$companyjob->id ? 'Create' : 'Update' }} </li>
    </x-admin.layouts.adminbreadcrumb>
    ...
    <x-admin.layouts.admincreateorupdate>
        ...
        <x-slot name="form">
            {!! Form::model($companyjob, ['route' => ['companyjob.store', $companyjob->id], 'id' => '', 'class' => 'row g-3 form_prevent_multiple_submits ', 'novalidate' => 'novalidate', 'files' => 'true', 'enctype' => 'multipart/form-data']) !!}
            {{ Form::hidden('id', $companyjob->id, ['id' => 'invisible_id']) }}
            {{ Form::hidden('uniqid', $companyjob->uniqid, ['id' => 'invisible_id']) }}


            @include('admin.job.jobmaster.companyjob.form')
            ...
            function checkCountTitle() {
                ...
                if (textLengthTitle >= maxCharstitle) {
                    ...
                    $('#descriptiontitleLeft').text('SEO DESCRIPTION  ' + countTitle +
                        ' characters left (Min:30,Max:200)');
                }
            }
