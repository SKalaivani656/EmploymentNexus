    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                ...
                @php
                    $categoryvideo = App\Models\Admin\Video\Categoryvideo::where('active', true)->get();
                @endphp
                @if ($categoryvideo->isNotEmpty())
                    @foreach ($categoryvideo as $eachcategoryvideo)
                        <li
                            class="nav-item {{ Str::contains(url()->current(), $eachcategoryvideo->slug) ? 'underline' : '' }}">
                            <a class="nav-link fw-bold active"
                                href="{{ route('categoryvideo', [$eachcategoryvideo->slug]) }}">{{ $eachcategoryvideo->name }}</a>
                        </li>
                    @endforeach
                @endif
        </div>
...
@php
$tagvideo = App\Models\Admin\Video\Tagvideo::where('active', true)->get();
@endphp
@if ($tagvideo->isNotEmpty())
    <div class="nav-scroller bg-body shadow-sm mb-3">
        <nav class="nav nav-underline" aria-label="Secondary navigation">
            @foreach ($tagvideo as $eachtagvideo)
                <a class="nav-link fw-bold" href="{{ route('tagvideo', [$eachtagvideo->slug]) }}"
                    style="color:rgb(19, 19, 105);">{{ $eachtagvideo->name }}</a>
            @endforeach
        </nav>
    </div>
@endif
